...
function constructQuery(state) {
    ...
    if (filters) {
        for (let key in filters) {
            let column = key !== 'checkboxes' ? columns[key] : {
                type: 'checkbox'
            }
...
function transformCount(json) {
    return {
        rowCount: json[0].count
    }
}

function transformApiMigration(json) {
    return {
        migrationId: json.nbeId
    }
}
...
export
const shouldRunColumnStats = (type, key) => {
