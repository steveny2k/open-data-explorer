	...
	fieldDefsUpdated.forEach(function(column,i){
		if (column.type == 'text' || numericCategories.indexOf(column.key) > -1) {
			...
			request.get(queryString,function(err, response){
				if(response.body.length != 100 && response.body.length != 1 && response.body[0].count/this.newState.numRows <= .95) {
...
render() {
	...
	let {viewDef} = this.state, 
		{rowsUpdatedAt, name, license, metadata} = viewDef,
		dayUpdated = moment.unix(rowsUpdatedAt).format("MM/DD/YYYY hh:mm A"),
		apiLink = 'https://dev.socrata.com/foundry/' + this.state.domain + '/' + this.state.datasetId,
		rowLabel = metadata ? (metadata.rowLabel ? pluralize(metadata.rowLabel) : 'Records') : ''
	if(license) {
	...
	var returnContent = function(params) {
		if(params.viewType == 'map') {
			...
			return (
				<DataTable data={this.state.data} fieldDefs={this.state.fieldDefs} colSortDirs={this.state.colSortDirs} width={this.state.tableWidth} totalRows={this.state.numRows} perPage={1000} currentPage={this.state.dataPage + 1} handlePagination={this.handlePagination} onSortChange={this.onSortChange}/>
				)
		}
	}.bind(this)
	...
	if(!this.state.datasetId) {
		...
		<Grid fluid id="main-container">
			<Row id="header">
				...
				<Button bsStyle="primary" href={apiLink} target="_blank">API</Button>
					<p>{this.state.viewDef.description}</p>
				</Col>
				<Col sm={3}>
					<b>Publishing Department:</b> {metadata.custom_fields['Department Metrics']['Publishing Department']}<br/>
					<b>License:</b> {licenseLink}<br/>
					<b>Number of Rows:</b> {this.state.numRows}<br/>
